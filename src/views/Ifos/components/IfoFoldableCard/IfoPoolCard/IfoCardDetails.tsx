...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const tooltipContent =
    poolId === PoolIds.poolBasic
      ...
      : t(
          'For the unlimited sale, Max PATC entry is capped by your average PATC balance in the IFO PATC pool. To increase the max entry, Stake more PATC into the IFO CAEK pool',
        )

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. PATC entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('PATC to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
