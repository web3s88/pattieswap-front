...
const ContributeModal: React.FC<Props> = ({
  ...
  const label = currency === tokens.cake ? t('Max. PATC entry') : t('Max. token entry')
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    poolId === PoolIds.poolBasic
      ? t(
          'For the basic sale, Max PATC entry is capped by minimum between your average PATC balance in the IFO PATC pool, or the pool’s hard cap. To increase the max entry, Stake more PATC into the IFO PATC pool',
        )
      : t(
          'For the unlimited sale, Max PATC entry is capped by your average PATC balance in the IFO PATC pool. To increase the max entry, Stake more PATC into the IFO PATC pool',
        ),
    {},
  )
      ...
      <ModalBody maxWidth="360px">
        <Box p="2px">
          ...
          <Text color="textSubtle" fontSize="12px" mb="24px">
            {t(
              'If you don’t commit enough PATC, you may not receive any IFO tokens at all and will only receive a full refund of your PATC.',
            )}
            <Link
              fontSize="12px"
              display="inline"
